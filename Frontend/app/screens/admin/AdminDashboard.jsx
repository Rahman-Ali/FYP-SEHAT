...
export default function AdminDashboard() {
  ...
  const loadDocuments = useCallback(async () => {
    try {
      ...
      // Safe check: ensure documents is an array
  ...
  const uploadDocument = useCallback(async (file) => {
    try {
      ...
      if (data.success) {
        setDocuments((prev) => [
          ...prev,
          { filename: file.name, indexed: false, chunk_count: 0 },
        ]);
  ...
  const handleRemoveDocument = useCallback((filename) => {
    Alert.alert("Remove Document", `Remove "${filename}"? This cannot be undone.`, [
      { text: "Cancel", style: "cancel" },
      {
        text: "Remove",
        style: "destructive",
        onPress: () => confirmRemoveDocument(filename),
      },
    ]);
  }, []);
  ...
  const renderItem = useCallback(
    ({ item }) => (
      <DocCard item={item} onRemove={handleRemoveDocument} isRemoving={isRemoving} />
    ),
    [handleRemoveDocument, isRemoving]
  );

  // Safe calculations
  const indexedCount = Array.isArray(documents)
      ...
      >
        ...
        {isUploading && (
          <View style={styles.uploadBanner}>
            ...
            <Text style={styles.uploadBannerText}>Uploading and indexing document...</Text>
          </View>
        )}
